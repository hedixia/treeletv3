...
class SA2_clust(SA_clust):
	def __init__ (self, dataset_ref, kernel, sample_para, inner_sample_para=False, slice=False, num_clust=0,
	              all_kernel=False):
		...
		for one_clust in layer1_cluster:
			if len(layer1_cluster[one_clust]) <= 1:
				continue
			layer2_clust = SA_clust(dataset_ref, kernel, inner_sample_para, layer1_cluster[one_clust], num_clust,
			                        all_kernel)
			layer2_clust.build()
